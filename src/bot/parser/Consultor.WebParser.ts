...
export class ConsultorWebParser implements IConsultorWebParser {
  ...
  private mapToInscripcionesAsistencia(
    row: string[]
  ): info_inscripciones_asistencia {
    return {
      materia: row[0],
  ...
  private mapToInfo_resultado_evaluacion_finales(
    row: string[]
  ): info_resultado_evaluacion_final {
    return {
      materia: row[0],
  ...
  private handleBykeyCases(key: string, rows: string[][]) {
    switch (key) {
      case this.table_map.info_inscripciones_asistencia:
        this.load_info(
          rows,
          this.mapToInscripcionesAsistencia,
          this.inscripciones
        );
        break;
      ...
      case this.table_map.info_resultado_parcial:
        this.load_info(
          rows,
          this.mapToInfo_resultado_parciales,
          this.resultado_parciales
        );
        break;
      ...
      case this.table_map.info_resultado_evaluacion_final:
        this.load_info(
          rows,
          this.mapToInfo_resultado_evaluacion_finales,
          this.finales
        );
        break;
      ...
      case this.table_map.info_materia_pendiente:
        this.load_info(
          rows,
          this.mapToInfo_materia_pendiente,
          this.materias_pendienetes
        );
        break;
      ...
      case this.table_map.info_horario_docente:
        this.load_info(
          rows,
          this.mapToInfo_horario_docente,
          this.horario_docentes
        );
        break;
      ...
      case this.table_map.info_libro_prestamo:
        this.load_info(
          rows,
          this.mapToInfo_libros_prestamo,
          this.libros_prestamo
        );
        break;
      default:
  ...
  public get_info_estudiante(): info_estudiante {
    const cabecera = this.selector("div.col-sm-7 > h3").text();
    let result: info_estudiante = {
      nombre: "*",
      apellido: "*",
      cedula: "*",
      cedula_nombre_apellido: cabecera,
    };
    return result;
  }
  public get_info_contacto(): info_contacto {
  ...
  public get_info_tiempo_rendimiento(): info_tiempo_rendimiento {
    const Pcarrera = this.selector('td:contains("Carrera...:")')
      .next()
      .text()
      .trim();
    const PfechaIngreso = this.selector('td:contains("Fecha de Ingreso...:")')
      .next()
      .text()
      .trim();
    const PfechaEgreso = this.selector(
      'td:contains("Fecha Estimada de Egreso...:")'
    )
      .next()
      .text()
      .trim();
    const Ppromedio = this.selector('td:contains("Promedio...:")')
      .next()
      .text()
      .trim();
    const PmateriasAprobadas = this.selector(
      'td:contains("Total Materias Aprobadas...:")'
    )
      .next()
      .text()
      .trim();
    const PmateriasReprobadas = this.selector(
      'td:contains("Total Materias Reprobadas...:")'
    )
      .next()
      .text()
      .trim();
    const PporcentajeReprobadas = this.selector(
      'td:contains("Porcentaje Materias Reprobadas...:")'
    )
      .next()
      .text()
      .trim();
    const PbeneficiarioLey = this.selector(
      'td:contains("Beneficiario/a de la ley 6628/2020")'
    )
      .text()
      .trim();
